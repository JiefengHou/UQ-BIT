<?php
	...
	if(isset($_POST['func']) && isset($_POST['id']) && $_POST['func']=="getdata"){
		...
		if($result){
			while($row = mysql_fetch_array($result)){
				echo $row['category']."  ".$row['name']."  ".$row['price']."  ".$row['dueTime']."  ".$row['location']."  ".$row['description']."  ".$row['imageLink']."  ".$row['comment'];
			}
		}
	}
	...
	if(isset($_POST['func']) && isset($_POST['id']) && isset($_POST['category']) 
		...
		&& isset($_POST['comment']) && $_POST['func']=="updatedata"){
		...
		mysql_query("UPDATE itemdata SET category = '{$_POST['category']}', name = '{$_POST['name']}',price = '{$_POST['price']}',
			...
			comment = '{$_POST['comment']}' WHERE id = '{$_POST['id']}'");
	}

	if(isset($_POST['func']) && isset($_POST['category']) 
		&& isset($_POST['name']) && isset($_POST['price']) && isset($_POST['dueTime']) 
		&& isset($_POST['location']) && isset($_POST['description']) && isset($_POST['imageLink']) 
		&& isset($_POST['comment']) && $_POST['func']=="adddata"){

		mysql_query("INSERT INTO itemdata (id, category, name, price, dueTime, location, 
			description, imageLink, comment) VALUES (NULL,'{$_POST['category']}','{$_POST['name']}',
			'{$_POST['price']}','{$_POST['dueTime']}','{$_POST['location']}','{$_POST['description']}', 
			'{$_POST['imageLink']}','{$_POST['comment']}') ");
	}
	...
	if(isset($_POST['func']) && isset($_POST['id']) && isset($_POST['input']) && $_POST['func']=="addcomment"){
		mysql_query("UPDATE itemdata SET comment = '{$_POST['input']}' WHERE id = '{$_POST['id']}' ");
		echo $_POST['input'];
	}

	if(isset($_POST['func']) && isset($_POST['id']) && $_POST['func']=="addmaker"){
		$result = mysql_query("SELECT * FROM itemdata WHERE id = '{$_POST['id']}'");
		if($result) {
			while($row = mysql_fetch_array($result)){
	...
	if(isset($_POST['input'])){
		...
		if(count(explode(" ",$_POST['input']))>1) {
			...
			if (strpos($input2, '>') !== false) {
				...
				if($result) {
					while($row = mysql_fetch_array($result)){
						echo $row['category']."  ".$row['name']."  ".$row['price']."  ".$row['dueTime']."  ".$row['location']."  ".$row['description']."  ".$row['imageLink']."  ".$row['comment']."  ".$row['Latitude']."  ".$row['Longitude']."@";
					}				
				}
