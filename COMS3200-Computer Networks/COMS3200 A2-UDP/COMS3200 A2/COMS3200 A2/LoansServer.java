...
public class LoansServer {
	...
	public static void main(String[] args)  throws Exception {
		...
		try {
			...
			LoansPort = serverSocket.getLocalPort(); 
			 ...
			 * function in new thread. If timeout, the client(loans server) will 
			 * re-send packet to NameServer.
			 * 
			 * receive accept msg from NameServer
			 * 
			 * If receive packet is timeout from NameServer is more than 5 times,
			 * it means client(loans server) cannot contact NameServer
			 */
			do{
				x=Math.random();	
				if(x>=0.5){
					sendData(clientSocket,ServerName+" "+LoansPort+" ",
							NameServerPort);
			...
			System.out.println("Loan Server is activated.\n");
			...
			while (true) {
				...
				//If the request type in msg is L or D
				...
				//If LoansServer receive invalid messages, the connection will 
				//close
				else {
					serverSocket.close();
				}		
